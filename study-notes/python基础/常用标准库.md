...
3. #### 装饰器元数据保存 wraps
   ...
   # DemoMock继承了Demo类，Demo.get方法使用了装饰器，在DemoMock中访问Demo未被装饰的原始get方法
   def auth(func):
       @functools.wraps(func)  # 使用wrpas装饰器保存函数原信息
       def inner(*args, **kwargs):
           print('===')
   ...
   class DemoMock(Demo):
       def get(self):
           # 直接访问未装饰的原始方法
           result = Demo.get.__wrapped__(self)  # 如果不适用functools.wraps保存函数原信息，那么无法访问未被装饰的原始方法
           # 添加额外的逻辑，如果需要的话
           print('在 DemoMock 的 get 方法中的额外逻辑')
           return result
   
   # 创建 DemoMock 的实例并调用重写的 get 方法
   dm = DemoMock().get()
...
a = {"template_id": "-1", "owner_type": 3, "category_id": 1, "file_id": 1}
demo = TemplateCategoryDeleteModel(**a)
print(demo.owner_type)  # 3 而不是枚举
```
