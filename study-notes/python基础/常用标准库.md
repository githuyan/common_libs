...
绝对路径  F:\python_pycharm\flask_learn\flask_venv

​				F:\\python_pycharm\\flask_learn\\flask_venv\\hellon.py

相对路径 F:/python_pycharm/flask_learn/flask_venv



暂时只能使用 .replace(origin,new) 替换了
...
os.path.abspath( path )   返回 path 的绝对路径  例 os.path.abspath( 'ddd\test' )

os.path.isabs( path )    是否是绝对路径 返回bool



os.path.basename( path )  -- 去掉目录路径获取文件名

os.path.dirname( path )   ---- 去掉文件名获取目录路径
