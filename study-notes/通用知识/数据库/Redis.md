...
- 复制原理
  ...
  master 接到命令，启动后台的存盘程序，同时手机素有接受到的用于修改数据集的命令，在后台进程执行完毕后，master将传送整个数据文件到slave，并完成一次完全同步
...
3. 数据预热，在正式部署之前，现将热点数据预先访问一遍，将这些数据都加在到缓存中，，在大并发发生前，手动触发缓存，设置不同的过期时间，让缓存失效的时间点尽量均匀
6. **注意**
...
redis和mongodb都是nosql数据库，在数据量不大的情况下，直接使用redis做缓存和持久化处理，

mongodb更适合海量数据的存储，它本身就是持久化数据库
